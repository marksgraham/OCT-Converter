...
def extract_fda_with_images_and_json(fda_file_path, output_dir):
    ...
    oct_volume.save(
        f"{output_dir}/{filepath.split('/')[-1].split('.')[0]}/{filepath.split('/')[-1].split('.')[0]}.avi")  # save volume as a movie
    oct_volume.save(
    ...
    fundus_image.save(f"{output_dir}/{filepath.split('/')[-1].split('.')[0]}/{filepath.split('/')[-1].split('.')[0]}_fundus.jpg")
        # save Fundus image to output directory, fda_filename_fundus.jpg

    fundus_grayscale_image = (
        fda.read_fundus_image_gray_scale()
    )  # returns a  Fundus image with additional metadata if available
    fundus_grayscale_image.save(f"{output_dir}/{filepath.split('/')[-1].split('.')[0]}/{filepath.split('/')[-1].split('.')[0]}_grayscale_fundus.jpg")
