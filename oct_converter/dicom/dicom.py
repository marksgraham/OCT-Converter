...
def populate_patient_info(ds: Dataset, meta: DicomMetadata) -> Dataset:
	...
	ds.PatientBirthDate = (
		meta.patient_info.patient_dob.strftime('%Y%m%d') if meta.patient_info.patient_dob else ""
	)
	return ds
...
meta.series_info.acquisition_date.strftime('%Y%m%d%H%M%S.%f') if meta.series_info.acquisition_date else ""
	)
